...
using QuickCode.DemoTest.OrderModule.Application.Services.OrderItem;
...
namespace QuickCode.DemoTest.OrderModule.Api.Controllers
{
    public partial class OrderItemsController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-by-order/{orderItemsOrderId:int}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetByOrderResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetByOrderAsync(int orderItemsOrderId, int? page, int? size)
        {
            if (page < 1)
            {
            ...
            var response = await service.GetByOrderAsync(orderItemsOrderId, page, size);
