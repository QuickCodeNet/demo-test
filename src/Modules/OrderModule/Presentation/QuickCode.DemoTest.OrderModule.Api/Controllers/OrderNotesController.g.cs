...
namespace QuickCode.DemoTest.OrderModule.Api.Controllers
{
    public partial class OrderNotesController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-by-order/{orderNotesOrderId:int}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetByOrderResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetByOrderAsync(int orderNotesOrderId, int? page, int? size)
        {
            if (page < 1)
            {
            ...
            var response = await service.GetByOrderAsync(orderNotesOrderId, page, size);
            if (HandleResponseError(response, logger, "OrderNote", $"OrderNotesOrderId: '{orderNotesOrderId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
    }
