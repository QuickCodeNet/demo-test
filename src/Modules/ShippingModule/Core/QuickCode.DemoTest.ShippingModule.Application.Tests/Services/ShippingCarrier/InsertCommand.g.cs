...
{
    public class InsertShippingCarrierCommandTests : IDisposable
    {
        ...
        public InsertShippingCarrierCommandTests()
        {
        ...
        public async Task InsertAsync_Should_Return_Success_When_Valid_Request()
        {
            ...
            var fakeResponse = new RepoResponse<ShippingCarrierDto>(fakeDto, "Success");
            _repositoryMock.Setup(r => r.InsertAsync(It.IsAny<ShippingCarrierDto>())).ReturnsAsync(fakeResponse);
            // Act
            var result = await _service.InsertAsync(fakeDto);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            Assert.Equal(fakeDto, result.Value);
            _repositoryMock.Verify(r => r.InsertAsync(It.IsAny<ShippingCarrierDto>()), Times.Once);
        }
        ...
        public async Task InsertAsync_Should_Return_NotFound_When_Repository_Returns_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<ShippingCarrierDto>("tr");
            var fakeResponse = new RepoResponse<ShippingCarrierDto>
            {
